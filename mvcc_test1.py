...
def generate_sql_files():
    """生成多个阶段的SQL文件（创建表、插入、更新、删除、插入、更新、清理）"""
    # 阶段1: 创建表并插入初始数据
    ...
    # 阶段3: 事务T2删除数据
    with open("stage3_t2_delete.sql", 'w') as f:
        f.write("BEGIN;\n")
        for i in range(1, 3):  # 删除前2条
            f.write(f"DELETE FROM test_table WHERE id = {i};\n")
        # 不提交，保持事务开启
        print(f"事务T2删除SQL文件已生成: stage3_t2_delete.sql")

    # 阶段4: 事务T3插入数据
    with open("stage4_t3_insert.sql", 'w') as f:
        f.write("BEGIN;\n")
        for i in range(1, 3):
            f.write(f"INSERT INTO test_table VALUES ({i}, 'T3_Inserted');\n")
        f.write("COMMIT;\n")  # 提交事务T3
        print(f"事务T3插入SQL文件已生成: stage4_t3_insert.sql")

    # 阶段5: 提交事务T1和T2
    with open("stage5_commit.sql", 'w') as f:
        f.write("COMMIT;\n")  # 提交事务T1
        f.write("COMMIT;\n")  # 提交事务T2
        print(f"提交事务T1和T2的SQL文件已生成: stage5_commit.sql")

    # 阶段6: 检查最终数据
    with open("stage6_check_data.sql", 'w') as f:
        f.write("SELECT * FROM test_table;\n")
        print(f"数据检查SQL文件已生成: stage6_check_data.sql")

    # 阶段7: 清理
    with open("stage7_cleanup.sql", 'w') as f:
        f.write("DROP TABLE test_table;\n")
        print(f"清理SQL文件已生成: stage7_cleanup.sql")
...
if __name__ == "__main__":
    ...
    run_sql_file(client_path, "stage3_t2_delete.sql", output_file, "事务T2删除数据（保持开启）")
    run_sql_file(client_path, "stage4_t3_insert.sql", output_file, "事务T3插入数据")
    run_sql_file(client_path, "stage5_commit.sql", output_file, "提交事务T1和T2")
    run_sql_file(client_path, "stage6_check_data.sql", output_file, "检查最终数据")
    run_sql_file(client_path, "stage7_cleanup.sql", output_file, "清理")
