...
private:
    ...
    /** 存储表堆中每个元组的先前版本。 */
    std::unordered_map<int, std::shared_ptr<PageVersionInfo>> version_info_;
...
public:
    ...
    std::optional<RmRecord> GetVisibleRecord(const Rid &rid, Transaction *current_txn);
    std::optional<RmRecord> GetVisibleRecord(std::shared_ptr<PageVersionInfo> page_info_ptr,
                                             const Rid &rid, Transaction *current_txn);
    std::shared_ptr<PageVersionInfo> GetPageVersionInfo(int page_no);
...
private:
    ...
    void TruncateVersionChain(const Rid &rid, timestamp_t watermark);
    void TruncateVersionChain(std::shared_ptr<PageVersionInfo> page_info,
                              int slot_no, timestamp_t watermark);

    void DeleteVersionChain(const Rid &rid);
    void DeleteVersionChain(std::shared_ptr<PageVersionInfo> page_info, const Rid &rid);

    bool UpdateUndoLink(const Rid &rid, UndoLog *prev_link);
};