...
{
    friend class RmFileHandle_Final;
...
public:
    RmManager_Final(DiskManager_Final *disk_manager, BufferPoolManager_Final *buffer_pool_manager)
     ...
     * @return {unique_ptr<RmFileHandle_Final>} 文件句柄的指针
     */
    std::shared_ptr<RmFileHandle_Final> open_file(const std::string &filename);
    /**
     * @description: 关闭表的数据文件
     * @param {RmFileHandle_Final*} file_handle 要关闭文件的句柄
     */
    void close_file(const RmFileHandle_Final *file_handle, bool flush = true);
};
