...
#include "rm_scan_final.h"
#include "rm_file_handle_final.h"
#include "storage/buffer_pool_manager_final.h"
#include "transaction/transaction_manager.h"

RmScan_Final::RmScan_Final(std::shared_ptr<RmFileHandle_Final> file_handle, Context *context) : file_handle_(file_handle),
                                                                                                context_(context),
...
void RmScan_Final::next_batch()
{
    load_next_page();
}

void RmScan_Final::load_next_page()
{
    // 移动到下一页
    rid_.page_no++;
    current_record_idx_ = 0;
    ...
    auto version_info = txn_manager->GetPageVersionInfo(
        PageId_Final{file_handle_->GetFd(), rid_.page_no});
...
bool RmScan_Final::is_end() const
{
    return rid_.page_no >= page_num ||
           current_records_.empty();
}

Rid RmScan_Final::rid() const
{
    return rid_;
}

std::vector<Rid> RmScan_Final::rid_batch() const
{
    std::vector<Rid> rids;
...
std::vector<std::unique_ptr<RmRecord>> RmScan_Final::record_batch()
{
