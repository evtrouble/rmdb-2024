...
public:
    ...
    std::shared_ptr<PortalStmt> start(std::shared_ptr<Plan> plan, Context *context)
    {
        ...
        else if (auto x = std::dynamic_pointer_cast<DMLPlan>(plan))
        {
            ...
            case T_select:
            {
                if (!x->subplan_)
                {
                    throw std::runtime_error("Subplan is null");
                }

                std::shared_ptr<ProjectionPlan> p = std::dynamic_pointer_cast<ProjectionPlan>(x->subplan_);
                if (!p)
                {
                    throw std::runtime_error("Failed to cast subplan to ProjectionPlan");
                }
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context)
    {
        if (!plan)
        {
            throw std::runtime_error("Plan is null");
        }
        ...
        if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan))
        {
            if (!x->subplan_)
            {
                throw std::runtime_error("ProjectionPlan subplan is null");
            }

            auto subplan_executor = convert_plan_executor(x->subplan_, context);
            if (!subplan_executor)
            {
                throw std::runtime_error("Failed to create subplan executor");
            }

            try
            {
                auto proj_executor = std::make_unique<ProjectionExecutor>(std::move(subplan_executor), x->sel_cols_);
        ...
        else if (auto x = std::dynamic_pointer_cast<ScanPlan>(plan))
        {
            ...
            if (x->tag == T_SeqScan)
            {
                try
                {
                    auto executor = std::make_unique<SeqScanExecutor>(sm_manager_, x->tab_name_, x->conds_, context);
                    return executor;
                }
                catch (const std::exception &e)
                {
