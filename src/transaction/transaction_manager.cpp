                                        ...
                                        std::function<bool(UndoLog *)> &&check)
{
    ...
    PageId_Final page_id{fd, rid.page_no};
...
void TransactionManager::DeleteVersionChain(std::shared_ptr<PageVersionInfo> page_info,
                                            const PageId_Final &page_id, const Rid &rid)
{
...
std::shared_ptr<PageVersionInfo> TransactionManager::GetPageVersionInfo(const PageId_Final &page_id)
{
    // 获取页面版本信息
    std::shared_lock global_read_lock(version_info_mutex_);
    auto page_it = version_info_.find(page_id);
...
{
    std::vector<std::shared_ptr<RmFileHandle_Final>> tables;
