...
public:
    ...
    std::shared_ptr<PageVersionInfo> GetPageVersionInfo(const PageId_Final &page_id);
    ...
    void DeleteVersionChain(std::shared_ptr<PageVersionInfo> page_info, const PageId_Final &page_id, const Rid &rid);
    ...
    /** 存储表堆中每个元组的先前版本。 */
    std::unordered_map<PageId_Final, std::shared_ptr<PageVersionInfo>, PageIdHash_Final> version_info_;
