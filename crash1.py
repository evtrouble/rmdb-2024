...
def generate_sql_files():
    ...
    # 生成静态检查点SQL文件
    with open("stage2_5_static_checkpoint.sql", 'w') as f:
        f.write("create static_checkpoint;\n")
        print(f"静态检查点SQL文件已生成: stage2_5_static_checkpoint.sql")
...
if __name__ == "__main__":
    ...
    if stage_times["事务操作"] >= 0:
        stage_times["静态检查点"] = run_stage(client_path, "静态检查点", "stage2_5_static_checkpoint.sql", output_file)

    # 阶段3: 崩溃前查询
    if stage_times.get("静态检查点", -1) >= 0:
