...
def generate_sql_files():
    ...
    with open("stage1_insert.sql", 'w') as f:
        f.write("CREATE TABLE warehouse (w_id INT, name CHAR(8), flo FLOAT);\n\n")  # 添加PRIMARY KEY约束
        print("正在生成INSERT语句...")
        for i in range(1, 3001):
            unique_str = f"{i:08d}"[-8:]
            # 按照等差数列生成flo的值，公差为0.1
            flo_value = 1024.5 + (i - 1) * 0.1
            f.write(f"INSERT INTO warehouse VALUES ({i}, '{unique_str}', {flo_value});\n")
    ...
    with open("stage2_no_index.sql", 'w') as f:
        ...
        for i in range(1, 3001):
            flo_value = 1024.5 + (i - 1) * 0.1
            f.write(f"SELECT * FROM warehouse WHERE w_id = {i} AND flo = {flo_value:.6f};\n")
        f.write("-- 无索引查询测试结束\n")
        print(f"无索引查询SQL文件已生成: stage2_no_index.sql")

    # 阶段3: 创建索引
    with open("stage3_create_index.sql", 'w') as f:
        f.write("CREATE INDEX warehouse(w_id, flo);\n")
    ...
    with open("stage5_delete.sql", 'w') as f:
        ...
        for i in range(1, 3001):
            flo_value = 1024.5 + (i - 1) * 0.1
            f.write(f"DELETE FROM warehouse WHERE w_id = {i} AND flo = {flo_value:.6f};\n")
        f.write("-- 删除测试结束\n")
